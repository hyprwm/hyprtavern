...
std::expected<std::string, std::string> GUI::firstTimeSetup() {
    ...
    auto           window = CWindowBuilder::begin()->preferredSize(WINDOW_SIZE)->minSize(WINDOW_SIZE)->maxSize(WINDOW_SIZE)->appTitle("KV Store Setup")->appClass("hyprtavern-kv")->commence();
    ...
    state.title =
        CTextBuilder::begin()->text("Hyprtavern KV store setup")->fontSize(CFontSize{CFontSize::HT_FONT_H2})->color([] { return backend->getPalette()->m_colors.text; })->commence();

    state.text =
        CTextBuilder::begin()->text(INFO_BOX_TEXT)->fontSize(CFontSize{CFontSize::HT_FONT_TEXT})->color([] { return backend->getPalette()->m_colors.text; })->async(false)->commence();
